...
void parse(char* line, command_t* p_cmd) {
    ...
    p_cmd->argc = temp_argc;
    alloc_mem_for_command(p_cmd, p_cmd->argc);
    int index = 0;
    char copy_line_again[strlen(line)];
    strcpy(copy_line_again, line);
    char *argument = strtok(copy_line_again, " ");
    while (argument!=NULL) {
        strcpy(p_cmd->argv[index], argument);
        index += 1;
...
bool find_fullpath(command_t* p_cmd) {
    ...
    char* path_env_variable;
    path_env_variable = getenv("PATH");
    char* file = p_cmd->argv[0];
    char copy_path[strlen(path_env_variable)];
    strcpy(copy_path, path_env_variable);
    char* path = strtok(copy_path, PATH_SEPARATOR);
    struct stat buff;
    while (path!=NULL) {
        char full_path[strlen(path)+MAX_ARG_LEN];
        strcpy(full_path, path);
        strcat(full_path, "/");
        strcat(full_path, file);
        if (stat(full_path, &buff) == 0 && (S_IFREG & buff.st_mode)) {
...
int execute(command_t* p_cmd) {
    ...
    if (is_builtin(p_cmd)) {
        ...
        if (find_fullpath(p_cmd)) {
            child_pid = fork();
            if (child_pid == 0) {
                status = execv(p_cmd->argv[0], p_cmd->argv);
            }
            else {
